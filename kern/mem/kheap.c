...
struct {
	struct pages Pages_arr[MAX_NUM_OF_PAGES];//List of all share variables created by any process
	struct spinlock kernel_lock;//Use it to protect the shares_list in the kernel
} k_arr;
...
int initialize_kheap_dynamic_allocator(uint32 daStart,
	...
	for (uint32 virtual_address = daStart; virtual_address < getSBrk();
			virtual_address += PAGE_SIZE) {
		struct FrameInfo *frame_info;
		if (allocate_frame(&frame_info) == 0) {
			map_frame(ptr_page_directory, frame_info, virtual_address,
					PERM_PRESENT | PERM_WRITEABLE);
			frame_info->VA = virtual_address;
		} else {
			for (uint32 allocated_va = daStart; allocated_va < virtual_address;
					allocated_va += PAGE_SIZE) {
...
void* sbrk(int numOfPages) {
	...
	if (new_s_brk > hard_limit)
		...
		for (uint32 va = old_s_brk; va < new_s_brk; va += PAGE_SIZE) {
			struct FrameInfo *frame_info;
			if (allocate_frame(&frame_info) == 0) {
				map_frame(ptr_page_directory, frame_info, va,
						PERM_PRESENT | PERM_WRITEABLE);
				frame_info->VA = va;
			} else {
				for (uint32 allocated_va = old_s_brk; allocated_va < va;
						allocated_va += PAGE_SIZE) {
					unmap_frame(ptr_page_directory, allocated_va);
				}
				return (void*) -1;
...
void* kmalloc(unsigned int size) {
	if (is_init == 0) {
		init_spinlock(&k_arr.kernel_lock, "kernel_lock");
		is_init = 1;
	}
	...
	if (isKHeapPlacementStrategyFIRSTFIT()) {
		...
		if (size <= DYN_ALLOC_MAX_BLOCK_SIZE) {
			bool holding_lock = holding_spinlock(&k_arr.kernel_lock);
			if (!holding_lock) {
				acquire_spinlock(&k_arr.kernel_lock);
			}
			void*vaa = alloc_block_FF((uint32) size);
			release_spinlock(&k_arr.kernel_lock);
			return vaa;
		...
		int count_wrabaad = 0;
		int firstpg = -1;
		uint32 MAX_PAGES = (KERNEL_HEAP_MAX - (getHardLimit() + PAGE_SIZE))
				/ PAGE_SIZE;
		for (uint32 i = 0; i < MAX_PAGES; i++) {
			if (k_arr.Pages_arr[i].allocated == 0) {
				if (count_wrabaad == 0) {
					firstpg = i;
				}
		...
		uint32 va_start_address = HARD_LIMIT + PAGE_SIZE
				+ (firstpg * PAGE_SIZE);
		...
		for (int i = firstpg; i < firstpg + pgs_needed; i++) {
			...
			map_res = map_frame(ptr_page_directory, frame, va,
					PERM_PRESENT | PERM_WRITEABLE);

			k_arr.Pages_arr[i].allocated = 1;
			frame->VA = va;
			va += PAGE_SIZE;
			if (map_res != 0 || allct_res != 0) {
				...
				for (int R = firstpg; R < i; R++) {
					...
					address += PAGE_SIZE;
					k_arr.Pages_arr[R].allocated = 0;
					k_arr.Pages_arr[R].psize = 0;
				}
				release_spinlock(&k_arr.kernel_lock);
				return NULL;
			}
			k_arr.Pages_arr[i].va = va_start_address;
			k_arr.Pages_arr[i].psize = size;
		}
...
void kfree(void* virtual_address) {
	...
	if ((uint32) virtual_address
			< getStartLimit() || (uint32)virtual_address >= KERNEL_HEAP_MAX) {
		panic("WHY?");
		return;
	}

	if ((uint32) virtual_address
			>= getSBrk() && (uint32)virtual_address < getHardLimit() + PAGE_SIZE) {
		return;
	}

	if ((uint32) virtual_address >= getStartLimit()
			&& (uint32) virtual_address < getSBrk()) {
		bool holding_lock = holding_spinlock(&k_arr.kernel_lock);
		if (!holding_lock) {
			acquire_spinlock(&k_arr.kernel_lock);
		}
		...
		if (k_arr.Pages_arr[page].allocated == 0) {
			release_spinlock(&k_arr.kernel_lock);
			return;
		}

		va = k_arr.Pages_arr[page].va;

		int allocated_pages = ROUNDUP(k_arr.Pages_arr[page].psize,
				PAGE_SIZE) / PAGE_SIZE;

		for (int i = page; i < page + allocated_pages; i++) {
			unmap_frame(ptr_page_directory, va);
			va += PAGE_SIZE;
			k_arr.Pages_arr[i].allocated = 0;
			k_arr.Pages_arr[i].psize = 0;
		}
